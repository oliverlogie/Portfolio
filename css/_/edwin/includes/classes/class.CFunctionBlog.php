...
/**
 * Object handling availability of comment functionality of contentitems.
 *
 ...
 */
class CFunctionBlog extends AbstractCFunction
{
  ...
  {
    if (in_array('blog', $this->_modules)) {
      return true;
    }
    else {
      return false;
  ...
  public function isAvailableForUser(User $user, NavigationSite $site)
  {
    ...
    $permitted = $user->AvailableModule('blog', $site->getID());
  ...
  {
    $excluded = ConfigHelper::get('m_blog_contenttype_excluded');
  ...
  {
    $paths = ConfigHelper::get('m_blog_from_page');
