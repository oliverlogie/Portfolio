 ...
 */
class ModuleAdminFeaturesModulesTypeFrontend extends Module
{
  protected $_prefix = 'ad_ft_mf';

  protected $_shortname = 'modules_frontend';
  ...
  protected function _initGrid()
  {
    ...
    $gridSql = " SELECT MID, MShortname, MClass, MActive "
             . " FROM {$this->table_prefix}moduletype_frontend "
             . " WHERE 1 ";
    ...
    $page = ($get->exists('ad_ft_mf_page')) ? $get->readInt('ad_ft_mf_page') : ($this->session->read('ad_ft_mf_page') ? $this->session->read('ad_ft_mf_page') : 1);
    $this->session->save('ad_ft_mf_page', $page);
  ...
  protected function _showContent()
  {
    ...
    $tplName = 'ad_ft_mf_content';
    $this->tpl->load_tpl($tplName, 'modules/ModuleAdminFeaturesModulesTypeFrontend.tpl');
    $this->tpl->parse_if($tplName, 'message', $this->_getMessage(), $this->_getMessageTemplateArray('ad_ft_mf'));
    ...
    $content = $this->tpl->parsereturn($tplName, array_merge( $this->_grid()->load_col_filters(), $this->_grid()->load_order_fields(), $this->_grid()->load_order_controls($this->_parseUrl(null, array('action3' => $this->_shortname))), array (
      'ad_ft_mf_action'                => $this->_parseUrl(null, array('action3' => $this->_shortname)),
      'ad_ft_mf_count_all'             => $this->_grid()->get_quantity_total_rows(),
      'ad_ft_mf_count_current'         => $currentRows,
      'ad_ft_mf_showpage_bottom'       => $this->_grid()->load_page_navigation($this->_parseUrl(null, array('action3' => $this->_shortname)) . '&amp;ad_ft_mf_page=','_bottom'),
      'ad_ft_mf_showpage_bottom_label' => sprintf($_LANG['m_grid_showpage_bottom_label'],($currentRows ? $currentSel['begin'] : 0),($currentRows ? $currentSel['end'] : 0)),
      'ad_ft_mf_showpage_top'          => $this->_grid()->load_page_navigation($this->_parseUrl(null, array('action3' => $this->_shortname)) . '&amp;ad_ft_mf_page=','_top'),
      'ad_ft_mf_showpage_top_label'    => sprintf($_LANG['m_grid_showpage_top_label'],($currentRows ? $currentSel['begin'] : 0),($currentRows ? $currentSel['end'] : 0)),
    ), $_LANG2['ad_ft_mf']));
  ...
  private function _processActivate()
  {
    ...
    $sql = " UPDATE {$this->table_prefix}moduletype_frontend "
         . "    SET MActive = ? "
         . "  WHERE MID = ? ";
    ...
    $this->_redirect(
      ...
      Message::createSuccess($active ? $_LANG['ad_ft_mf_message_activate_success'] : $_LANG['ad_ft_mf_message_deactivate_success'])
    );
  }
}