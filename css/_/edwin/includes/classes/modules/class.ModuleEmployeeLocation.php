...
/**
 * class.ModuleEmployeeLocation.php
 *
...
class ModuleEmployeeLocation extends AbstractModuleAttribute
{
  protected $_prefix = 'en';

  public function show_innercontent($subModuleShortname = '')
  {
    $attrGlobal = new AttributeGlobal($this->db, $this->table_prefix);
    $ident = $attrGlobal->readAttributeGlobalByIdentifier($this->_prefix.'_location');
    $this->_typeId = $ident->id;

    return parent::show_innercontent('location');
  }
  ...
  protected function _showForm()
  {
    ...
    $fieldsToHide = array('id', 'position', 'code', 'parentId', 'attributeLinkGroup', 'image',);
